...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('SELECT * FROM profiles WHERE email = $1 AND is_active = true', [email.toLowerCase().trim()]);
...
router.get('/me', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT id, email, role, full_name, display_name FROM profiles WHERE id = $1', [decoded.id]);
        if (result.rows.length === 0)
