...
async function main() {
  try {
    console.log('🚀 Starting Deep Cleanup and Real Data Restoration...');
    await client.connect();
    console.log('📡 Connected to database.');
    await client.query('BEGIN');
    console.log('🔓 Transaction started.');

    // 1. DELETE fake proposals completely! The user didn't want them.
    await client.query('DELETE FROM proposals');
    console.log('✅ Wiped out all fake proposals.');

    // 2. Clear Assignments & Invoices for fresh import
    await client.query('DELETE FROM invoices');
    await client.query('DELETE FROM assignments');
    console.log('✅ Cleared existing assignments and invoices.');

    // 3. Remove old "Mahindra and Mahindra Lt" client so it doesn't linger
    await client.query(`DELETE FROM clients WHERE name = 'Mahindra and Mahindra Lt'`);
    console.log('✅ Removed generic Mahindra client.');

    // Standardize Gmail Accounts
    const users = [
    ...
    const passwordHash = '$2a$10$uHfwPRTiaT4etSL/jjrsxupiFUWo/k2Pw0g5YgA3962OqD5kOCkvS';
    ...
    console.log('🎉 SUCCESS! Cleaned proposals and accurately restored Mahindra + Hamza Data.');
  } catch (err: any) {
