...
const PARTNER_ID = '00000000-0000-0000-0000-000000000005'; // Rishabh Thakkar (Director/Partner for Hamza)
...
async function main() {
  try {
    console.log('Restoring Hamza Momin data (V2)...');
    await pool.query('BEGIN');

    // 1. Delete existing assignments for Hamza to avoid duplication
    await pool.query('DELETE FROM invoices WHERE assignment_id IN (SELECT id FROM assignments WHERE manager_id = $1)', [MANAGER_ID]);
    await pool.query('DELETE FROM fee_allocations WHERE assignment_id IN (SELECT id FROM assignments WHERE manager_id = $1)', [MANAGER_ID]);
    ...
    for (const row of spreadsheetData) {
      ...
      // 4. Create an invoice to reflect "Billed Amount" in dashboard
      if (row.billedAmount > 0) {
        await pool.query(`
          INSERT INTO invoices (
            assignment_id, invoice_number, invoice_date, status, 
            professional_fees, net_amount, amount_received, created_by
          ) VALUES (
            $1, $2, NOW(), $3, $4, $4, $5, $6
          )
        `, [
          ...
          row.amountReceipt >= row.billedAmount ? 'paid' : (row.amountReceipt > 0 ? 'partially_paid' : 'sent'),
          row.billedAmount,
          row.amountReceipt,
          ADMIN_ID
        ]);
    ...
    console.log('✅ Successfully restored Hamza Momin data with invoices.');
  } catch (err) {
