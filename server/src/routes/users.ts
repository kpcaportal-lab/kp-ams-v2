...
router.get('/', requireRole('admin', 'partner', 'director'), async (_req: Request, res: Response) => {
    try {
        const result = await pool.query(
            'SELECT id, email, role, full_name, display_name, is_active, created_at FROM profiles ORDER BY role, full_name'
        );
...
router.patch('/:id', requireRole('admin', 'partner', 'director'), async (req: Request, res: Response) => {
    try {
        ...
        await pool.query(
            'UPDATE profiles SET is_active=COALESCE($1,is_active), role=COALESCE($2,role), display_name=COALESCE($3,display_name), updated_at=NOW() WHERE id=$4',
            [is_active, role, display_name, req.params.id]
        );
